...
ResultType TwitchVideoJob::Download(std::vector<std::string>& files,
    ...
    while (files.size() < downloadInfos.size()) {
        ...
        for (size_t i = 0; i < downloadInfos.size(); ++i) {
            ...
            if (success) {
                // await job.StallWrite(outpath, new FileInfo(outpath_temp).Length,
                // cancellationToken);
                if (cancellationToken.IsCancellationRequested()) {
                    return ResultType::Cancelled;
                }
...
ResultType TwitchVideoJob::Run(const std::string& targetFolderPath,
    ...
    if (!HyoutaUtils::IO::FileExists(std::string_view(targetFilename))) {
        if (!HyoutaUtils::IO::FileExists(std::string_view(combinedFilename))) {
            ...
            while (true) {
                ...
                ResultType downloadResult =
                    Download(files, cancellationToken, tempFolderForParts, downloadInfos);
        ...
        // await StallWrite(remuxedFilename, new FileInfo(combinedFilename).Length,
        // cancellationToken);
        if (cancellationToken.IsCancellationRequested()) {
            return ResultType::Cancelled;
        }
